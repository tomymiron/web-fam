...
<?php $sql = "SELECT * FROM `startedCompetitions` ORDER BY `startedCompetitions`.`date` DESC limit 1";
...
while($row = $result->fetch_assoc()){ 
    ...
    <style><?php include('../../public/resources/css/resultsP.css')?></style>
</head>
<body>
    ...
    <main>
        <aside id="menu">
            ...
            <a title="Resultados" class="option active" href="../resultados">
                <svg width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div id="boxedContainer">
                        <div id="boxedContainerAux">
                            ...
                            if(isset($_COOKIE['mainCompetitionResults'])){
                                $sql = "select competitionlog.result, disciplines.campus, disciplines.name from competitionlog left join events on events.ID_event = competitionlog.ID_event left join disciplines on events.ID_discipline = disciplines.ID_discipline left join competitions on competitions.ID_competition = events.ID_competition where competitions.ID_competition = " . $_COOKIE['mainCompetitionResults'] . " and competitionLog.ID_athlete = " . $user['ID_athlete'];
                            }else{
                                $sql = "select competitionlog.result, disciplines.campus, disciplines.name from competitionlog left join events on events.ID_event = competitionlog.ID_event left join disciplines on events.ID_discipline = disciplines.ID_discipline left join competitions on competitions.ID_competition = events.ID_competition where competitions.ID_competition = " . $mainCompetition . " and competitionLog.ID_athlete = " . $user['ID_athlete'];
                            }
                            $result = $conn->query($sql);
                            while($row = $result->fetch_assoc()){ ?>   
                            ...
                            <div class="boxed">
                                <div>
                                    <svg width="63" height="63" viewBox="0 0 65 66" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M20.9279 7.5625C17.9703 7.5625 15.5728 9.99697 15.5728 13V27.5C15.5728 36.9925 23.1513 44.6875 32.4998 44.6875C41.8485 44.6875 49.4269 36.9925 49.4269 27.5V13C49.4269 9.99697 47.0292 7.5625 44.0717 7.5625H20.9279Z" stroke="#2BCCA2" stroke-width="1.5"/>
                                        <path d="M32.5 46.75V49.5" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round"/>
                                        <path d="M40.6252 51.5625H24.3752C22.5054 51.5625 20.9897 53.1015 20.9897 55C20.9897 56.8985 22.5054 58.4375 24.3752 58.4375H40.6252C42.4949 58.4375 44.0106 56.8985 44.0106 55C44.0106 53.1015 42.4949 51.5625 40.6252 51.5625Z" stroke="#2BCCA2" stroke-width="1.5"/>
                                        <path d="M14.8958 13.75C11.1564 13.75 8.125 16.828 8.125 20.625V21.5282C8.125 23.5911 8.93206 25.5695 10.3687 27.0282L16.25 33" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M50.1042 13.75C53.8436 13.75 56.875 16.828 56.875 20.625V21.5282C56.875 23.5911 56.0679 25.5695 54.6314 27.0282L48.75 33" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M21.3649 8.52222C18.519 8.52222 16.2119 10.8648 16.2119 13.7545V27.7074C16.2119 36.8416 23.5044 44.2463 32.5002 44.2463C41.4961 44.2463 48.7886 36.8416 48.7886 27.7074V13.7545C48.7886 10.8648 46.4814 8.52222 43.6355 8.52222H21.3649Z" stroke="#2BCCA2" stroke-width="1.5"/>
                                        <path d="M32.5 46.231V48.8772" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round"/>
                                        <path d="M40.3188 50.8618H24.682C22.8828 50.8618 21.4243 52.3428 21.4243 54.1696C21.4243 55.9964 22.8828 57.4774 24.682 57.4774H40.3188C42.1179 57.4774 43.5764 55.9964 43.5764 54.1696C43.5764 52.3428 42.1179 50.8618 40.3188 50.8618Z" stroke="#2BCCA2" stroke-width="1.5"/>
                                        <path d="M15.5603 14.4763C11.9619 14.4763 9.04492 17.4382 9.04492 21.0919V21.961C9.04492 23.946 9.82152 25.8498 11.2039 27.2534L16.8633 32.9999" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M49.4398 14.4763C53.0381 14.4763 55.9551 17.4382 55.9551 21.0919V21.961C55.9551 23.946 55.1785 25.8498 53.7962 27.2534L48.1367 32.9999" stroke="#2BCCA2" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </div>
                                <div class="boxed1">
                                    <p class="boxedEvent"><?php echo($row['name']);?></p>
                                    <p class="boxedTime"><?php if($row['campus'] == 0){echo (getTime($row['result']) . "s");}else{echo ($row['result'] . "m");}?></p>
                                </div>
                            </div>
                            <?php } ?>
                        </div>
                                                ...
                                                <?php 
                                                    ...
                                                    while($getResultsRow = $resultGetResults->fetch_assoc()){
                                                        ...
                                                        if($auxResult == "DNS" || $auxResult == "DNF" || $auxResult == "DQ"){
                                                            if($getResultsRow['ID_athlete'] != $user['ID_athlete'] && $wind){
                                                                $auxWind = "<td class='resultWind'>-</td>";
                                                            }elseif($getResultsRow['ID_athlete'] == $user['ID_athlete'] && $wind){
                                                                $auxWind = "<td class='resultWind' style='color:#08ffff'>-</td>";
                                                            }else{
                                                                $auxWind = "";
                                                            }
                                                            if($getResultsRow['ID_athlete'] != $user['ID_athlete']){
                                                                ...
                                                                array_push($resultsToEcho,  "<tr>
                                                                        <td class='listNumber' style='color:#08ffff'>-</td>
                                                                        <td style='color:#08ffff'>".$getResultsRow['name']."</td>
                                                                        <td style='color:#08ffff'>".$getResultsRow['year']."</td>
                                                                        <td style='color:#08ffff'>".$getResultsRow['club']."</td>
                                                                        <td class='d-none d-xl-table-cell resultFed'>".$getResultsRow['federation']."</td>$auxWind
                                                                        <td style='color:#08ffff'>". $auxResult . "</td>
                                                                    </tr>");
                                                            }
                                                        }else{
                                                            if($getResultsRow['ID_athlete'] != $user['ID_athlete'] && $wind){
                                                                ...
                                                                $auxWind = "<td class='resultWind' style='color:#08ffff'>".$getResultsRow['wind']."</td>";
                                                            }else{
                                                                $auxWind = "";
                                                            }
                                                            if($getResultsRow['ID_athlete'] != $user['ID_athlete']){
                                                                ...
                                                                array_unshift($resultsToEcho, "</td>
                                                                        <td class='resultName' style='color:#08ffff'>".$getResultsRow['name']."</td>
                                                                        <td class='resultYear' style='color:#08ffff'>".$getResultsRow['year']."</td>
                                                                        <td class='resultClub' style='color:#08ffff'>".$getResultsRow['club']."</td>
                                                                        <td class='d-none d-xl-table-cell resultFed' style='color:#08ffff'>".$getResultsRow['federation']."</td>$auxWind
                                                                        <td class='resultResult' style='color:#08ffff'>". $auxResult . "</td>
                                                                    </tr>");
                                                                    $flagOwnUser = true;
                                                                    $ownUserPos = 0;
                                                            }
                                                    ...
                                                    for($i = 0; $i < count($resultsToEcho); $i++){
                                                        if($i < $resultPos){
                                                            if($flagOwnUser && $i == $ownUserPos){
                                                                echo "<tr>
                                                                <td class='listNumber' style='color:#08ffff'>" . $i + 1;
                                                                echo $resultsToEcho[$i];
                                                            }else{
                                                                echo "<tr>
                                                                <td class='listNumber'>" . $i + 1;
                                                                echo $resultsToEcho[$i];
                                                            }   
                                                        }else{
                                                            echo $resultsToEcho[$i];
    ...
    <script> <?php include('../../public/resources/js/resultsP.js'); ?> </script>
