...
$results = '';
$query = "select athletes.ID_athlete, clubs.ID_club, athletes.name, clubs.resumed as 'club', images.image from athletes left join clubs on clubs.ID_club = athletes.ID_club left join images on images.ID_athlete = athletes.ID_athlete where (athletes.dni < 5000) and (athletes.dni > 100) and (clubs.name like '%$name%' or clubs.resumed like '%$name%') order by athletes.name ASC limit 21;";
...
while($row = $result->fetch_assoc()){
    ...
    $results .= "
        <button type='submit' name='user' value='" . $row['ID_club'] . "' class='clubItem col-6 col-sm-4 col-lg-3 col-xl-2'>
            <div class='clubItemContainer'>
                <div class='clubPhoto'>
                    $imageAux
                </div>
                <p class='clubClub'>" . $row['club'] . "</p>
                <p class='clubName'>" . $row['name'] . "</p>
            </div>
        </button>
        ";
}
