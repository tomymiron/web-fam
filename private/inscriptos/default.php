...
while($row = $result->fetch_assoc()){ 
    ...
    <script>document.cookie = "mainCompetitionInscriptions=<?php echo($mainCompetition); ?>"</script>
    <?php
} ?>

    <title>Inscriptos</title>
    <style><?php include('../../public/resources/css/inscribed.css')?></style>
...
<body>
            ...
            <div id="mainContent" class="row container-fluid">
                ...
                <div id="leftSide" class="col-12 col-lg-3">
                    ...
                    <div id="box2" class="box">
                        ...
                        <div id="newInscriptionContainer">
                            <form id="newInscription" spellcheck="false">
                                <input id="newInscriptionDNI" placeholder="DNI atleta" type="number">
                                <input id="newInscriptionCompetition" type="hidden" value="<?php if(isset($_COOKIE['mainCompetitionInscriptions'])){echo($_COOKIE['mainCompetitionInscriptions']);}else{echo($mainCompetition);}?>">
                                <select id="newInscriptionEvent">
                                    <?php   
                                    if(isset($_COOKIE['mainCompetitionInscriptions'])){
                                        $auxInscriptionQuery = "select events.ID_event, events.gender, disciplines.name from events left join disciplines on events.ID_discipline = disciplines.ID_discipline where ID_competition = " . $_COOKIE['mainCompetitionInscriptions'] . " order by disciplines.name ASC, events.time ASC";
                                    }else{
                                        $auxInscriptionQuery = "select events.ID_event, events.gender, disciplines.name from events left join disciplines on events.ID_discipline = disciplines.ID_discipline where ID_competition = " . $mainCompetition . " order by disciplines.name ASC, events.time ASC";
                                    }
                                    $auxInscriptionResult = $conn->query($auxInscriptionQuery);
                                    while($auxInscription = $auxInscriptionResult->fetch_assoc()){ ?>
                                    <option name="selection" value="<?php echo($auxInscription['ID_event'])?>"><?php echo($auxInscription['name'])?> | <?php if($auxInscription['gender'] == 0){echo "F";}else{echo "M";}?></option>
                                    <?php } ?>
                                </select>
                                <button type="button" onclick="newInscription()">Aceptar</button>
                            </form>  
                        </div>
                    </div>
                    ...
                    <div id="box4" class="box">
                        <div id="box4ContentContainer">
                            <div id="box4Content">
                                <h2>Inscriptos auxiliares</h2>
                            <?php
                            if(isset($_COOKIE['mainCompetitionInscriptions'])){
                                $auxiliarInscriptionsQuery = "select inscriptions.ID_inscription, athletes.name, year(athletes.born) as 'year', disciplines.name as 'discipline', inscriptions.record from inscriptions left join events on events.ID_event = inscriptions.ID_event left join competitions on events.ID_competition = competitions.ID_competition right join inscriptionsauxiliar on inscriptionsauxiliar.ID_inscription = inscriptions.ID_inscription left join athletes on inscriptions.ID_athlete = athletes.ID_athlete left join disciplines on events.ID_discipline = disciplines.ID_discipline where competitions.ID_competition = " . $_COOKIE['mainCompetitionInscriptions'] . " order by disciplines.name ASC";
                            }else{
                                $auxiliarInscriptionsQuery = "select inscriptions.ID_inscription, athletes.name, year(athletes.born) as 'year', disciplines.name as 'discipline', inscriptions.record from inscriptions left join events on events.ID_event = inscriptions.ID_event left join competitions on events.ID_competition = competitions.ID_competition right join inscriptionsauxiliar on inscriptionsauxiliar.ID_inscription = inscriptions.ID_inscription left join athletes on inscriptions.ID_athlete = athletes.ID_athlete left join disciplines on events.ID_discipline = disciplines.ID_discipline where competitions.ID_competition = " . $mainCompetition . " order by disciplines.name ASC";
                            }
                            $auxiliarInscriptionsResults = $conn->query($auxiliarInscriptionsQuery); 
                            
                                while($auxiliarInscriptionsRow = $auxiliarInscriptionsResults->fetch_assoc()){ ?>
                                <div class="auxiliarInscriptionsItem">
                                    <div class="auxiliarInscriptionsItem1">
                                        <h5><?php echo($auxiliarInscriptionsRow['name']);?> <span><?php echo($auxiliarInscriptionsRow['year']);?></span></h5>
                                        <p><?php echo($auxiliarInscriptionsRow['discipline']);?>, <?php if($auxiliarInscriptionsRow['record'] == NULL){echo "sinMarca";}else{echo $auxiliarInscriptionsRow['record'];};?></p>
                                    </div>
                                    <div onclick="deleteAuxiliarInscriptionItem(<?php echo($auxiliarInscriptionsRow['ID_inscription']);?>)" class="auxiliarInscriptionsItem2">
    ...
    <script><?php include('../../public/resources/js/inscribed.js');?></script>
