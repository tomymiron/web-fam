...
while($row = $result->fetch_assoc()){ 
    ...
    <script>document.cookie = "mainCompetitionResults=<?php echo($mainCompetition); ?>"</script>
    <?php
} ?>

    <title>Resultados</title>
    <style><?php include('../../public/resources/css/results.css')?></style>
...
<body>
    ...
    <main>
        <aside id="menu">
            ...
            <a title="Resultados" class="option active">
                <svg width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div id="mainContent" class="row container-fluid">
                ...
                <div id="leftSide" class="col-12 col-lg-3">
                    <div id="box1" class="box">
                        <h2>Buscar competencia</h2>
                        <form id="viewForm" spellcheck="false">
                            <select id="selectInscription">
                                <?php $sql = "select * from competitions where public = 1 order by date desc limit 15";
                                $result = $conn->query($sql);
                                while($row = $result->fetch_assoc()){ ?>
                    ...
                    <div id="box2" class="box">
                        ...
                        <div id="newResultContainer">
                            <form id="newResult" spellcheck="false">
                                <input id="newResultDNI" placeholder="DNI atleta" type="number">
                                <select id="newResultEvent">
                                    <?php   
                                    if(isset($_COOKIE['mainCompetitionResults'])){
                                        $auxResultQuery = "select events.ID_event, events.gender, disciplines.name from events left join disciplines on events.ID_discipline = disciplines.ID_discipline where ID_competition = " . $_COOKIE['mainCompetitionResults'] . " order by disciplines.name ASC, events.gender ASC";
                                    }else{
                                        $auxResultQuery = "select events.ID_event, events.gender, disciplines.name from events left join disciplines on events.ID_discipline = disciplines.ID_discipline where ID_competition = " . $mainCompetition . " order by disciplines.name ASC, events.gender ASC";
                                    }
                                    $auxResultResult = $conn->query($auxResultQuery);
                                    while($auxResult = $auxResultResult->fetch_assoc()){ ?>
                                    <option name="selection" value="<?php echo($auxResult['ID_event'])?>"><?php echo($auxResult['name'])?> | <?php if($auxResult['gender'] == 0){echo "F";}else{echo "M";}?></option>
                                    <?php } ?>
                                </select>
                                <input id="newResultResult" placeholder="Resultado" type="number" step='any'>
                                <input id="newResultWind" placeholder="Viento" type="number" step='any'>
                                <button type="button" onclick="newResult()">Aceptar</button>
                            </form>  
                        </div>
                    </div>
                    <div id="box4" class="box">
                        <h2>Resultados Auxiliares</h2>
                        <div id="box4ContentContainer">
                            <div id="box4Content">
                            <?php
                            if(isset($_COOKIE['mainCompetitionResults'])){
                                $auxiliarResultsQuery = "select competitionLog.ID_competitionLog, athletes.name, year(athletes.born) as 'year', disciplines.name as 'discipline', competitionLog.result from competitionlog left join events on events.ID_event = competitionlog.ID_event left join competitions on events.ID_competition = competitions.ID_competition right join competitionlogauxiliar on competitionlogauxiliar.ID_competitionLog = competitionLog.ID_competitionLog left join athletes on competitionlog.ID_athlete = athletes.ID_athlete left join disciplines on events.ID_discipline = disciplines.ID_discipline where competitions.ID_competition = " . $_COOKIE['mainCompetitionResults'] . " order by disciplines.name ASC";
                            }else{
                                $auxiliarResultsQuery = "select competitionLog.ID_competitionLog, athletes.name, year(athletes.born) as 'year', disciplines.name as 'discipline', competitionLog.result from competitionlog left join events on events.ID_event = competitionlog.ID_event left join competitions on events.ID_competition = competitions.ID_competition right join competitionlogauxiliar on competitionlogauxiliar.ID_competitionLog = competitionLog.ID_competitionLog left join athletes on competitionlog.ID_athlete = athletes.ID_athlete left join disciplines on events.ID_discipline = disciplines.ID_discipline where competitions.ID_competition = " . $mainCompetition . " order by disciplines.name ASC";
                            }
                            $auxiliarResultsResults = $conn->query($auxiliarResultsQuery); 
                            while($auxiliarResultsRow = $auxiliarResultsResults->fetch_assoc()){ ?>
                                <div class="auxiliarResultsItem">
                                    <div class="auxiliarResultsItem1">
                                        <h5><?php echo($auxiliarResultsRow['name']);?> <span><?php echo($auxiliarResultsRow['year']);?></span></h5>
                                        <p><?php echo($auxiliarResultsRow['discipline']);?>, <?php echo($auxiliarResultsRow['result']);?></p>
                                    </div>
                                    <div onclick="deleteAuxiliarResultItem(<?php echo($auxiliarResultsRow['ID_competitionLog']);?>)" class="auxiliarResultsItem2">
                                        <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M26.25 7.47485C22.0875 7.06235 17.9 6.84985 13.725 6.84985C11.25 6.84985 8.775 6.97485 6.3 7.22485L3.75 7.47485" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M10.625 6.2125L10.9 4.575C11.1 3.3875 11.25 2.5 13.3625 2.5H16.6375C18.75 2.5 18.9125 3.4375 19.1 4.5875L19.375 6.2125" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M23.5623 11.425L22.7498 24.0125C22.6123 25.975 22.4998 27.5 19.0123 27.5H10.9873C7.49976 27.5 7.38726 25.975 7.24976 24.0125L6.43726 11.425" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                            <path d="M12.9121 20.625H17.0746" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                        ...
                                        <div class="resultItemOpen collapse" id="result<?php echo($row['ID_event']);?>">
                                            <div class="auxiliar">
                                                ...
                                                for($i = 0; $i < count($heatsAmount); $i++){
                                                    echo "<form class='heatContainer'  id='heat-".$row['ID_event']."-".$i."' spellcheck='false'>";
                                                    echo "<div class='heatContainerAux' id='heatAuxiliar-".$row['ID_event']."-".$i."'>";
                                                    echo "<h5>Serie " . $i + 1 . "</h5>";
                                                    echo "<div class='listResults'>
                                                            <table>
                                                                <tr>
                                                                    <th class='listNumber'>N°</th>
                                                                    <th class='tableName'>Nombre</th>
                                                                    <th class='tableYear'>Año</th>
                                                                    <th class='tableClub d-none d-md-block'>Club</th>
                                                                </tr>";
                                                    ...
                                                    while($heat = $heatsResult->fetch_assoc()){
                                                        $searchQuery = "select * from competitionLog where ID_event = " . $row['ID_event'] . " and ID_athlete = " . $heat['ID_athlete'];
                                                        $resultSearch = $conn->query($searchQuery);
                                                        if(mysqli_num_rows($resultSearch) > 0){
                                                            while($searchRow = $resultSearch->fetch_assoc()){
                                                                $auxiliarResutInput = "<input type='number' step='any' value='" . $searchRow['result'] . "' athlete='".$heat['ID_athlete']."' name='result-".$row['ID_event']."-".$i."'>";
                                                                $heatWindAux = $searchRow['wind'];
                                                            }
                                                        }else{
                                                            $auxiliarResutInput = "<input type='number' placeholder='--' step='any' athlete='".$heat['ID_athlete']."' name='result-".$row['ID_event']."-".$i."'>";
                                                        }

                                                        $outputAux[$reference[$heatsAux]] = "<tr>
                                                                                                <td class='listNumber'>$reference[$heatsAux]</td>
                                                                                                <td class='tableName'>".$heat['name']."</td>
                                                                                                <td class='tableYear'>".$heat['year']."</td>
                                                                                                <td class='tableClub d-none d-md-block'>".$heat['club']."</td><td class='heatContainerInput'>" . $auxiliarResutInput . "</td></tr>";
                                                        $heatsAux++;
                                                    }
                                                    ...
                                                    if($row['name'] == "50m" or $row['name'] == "60m" or $row['name'] == "80m" or $row['name'] == "100m" or $row['name'] == "150m" or $row['name'] == "200m" or $row['name'] == "60c/v" or $row['name'] == "80c/v" or $row['name'] == "100c/v" or $row['name'] == "110c/v"){
                                                        if($heatWindAux != null){
                                                            echo "<input class='windInput' value='$heatWindAux' type='number' step='any' id='wind-".$row['ID_event']."-".$i."'>";
                                                        }else{
                                                            echo "<input class='windInput' placeholder='Viento' type='number' step='any' id='wind-".$row['ID_event']."-".$i."'>";
                                                        }
                                                    }
                                                    echo "<button class='sendResultsButton' type='button' onclick='sendResults(".$row['ID_event'].",".$i.")'>Aceptar</button>";
                                                    echo "</div></div>";
                                                    echo "</form>";
    ...
    <script> <?php include('../../public/resources/js/results.js'); ?> </script>
